...
object IO2aTests {
  ...
  val g: Int => IO[Int] =
    List.fill(10000)(f).foldLeft(f) {
      (a: Int => IO[Int], b: Int => IO[Int]) => (x: Int) => IO.suspend(a(x).flatMap(b))
    }
...
object IO2bTests {
  ...
  val g: Int => TailRec[Int] =
    List.fill(10000)(f).foldLeft(f) {
      (a: Int => TailRec[Int], b: Int => TailRec[Int]) => (x: Int) => TailRec.suspend(a(x).flatMap(b))
    }
...
object IO3 {
  ...
  */
  ...
  def run[F[_], A](a: Free[F, A])(implicit F: Monad[F]): F[A] = step(a) match {
    ...
    case Suspend(r) => r
    case FlatMap(x, f) => x match {
      case Suspend(s) => F.flatMap(s)(t => run(f(t)))
      case _ => sys.error("Impossible, since `step` eliminates these cases")
    }
  }
  ...
  */
  ...
  case object ReadLine extends Console[Option[String]] {
    ...
    def toState = ConsoleState { buffers =>
      buffers.in match {
        case List() => (None, buffers)
        case h :: t => (Some(h), buffers.copy(in = t))
      }
    }
  ...
  case class PrintLine(line: String) extends Console[Unit] {
    ...
    def toReader = ConsoleReader { _ => () } // no-op
    def toState = ConsoleState { buffers => ((), buffers.copy(out = buffers.out :+ line)) } // append to the output
  }
  ...
  object ConsoleState {
    ...
    implicit val monad = new Monad[ConsoleState] {
      def unit[A](a: => A): ConsoleState[A] =
        ConsoleState(buffers => (a, buffers))

      def flatMap[A, B](ra: ConsoleState[A])(f: A => ConsoleState[B]): ConsoleState[B] =
        ra.flatMap(f)
    }
