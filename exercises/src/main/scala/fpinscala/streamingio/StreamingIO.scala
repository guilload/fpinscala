...
object SimpleStreamTransducers {
  ...
  sealed trait Process[I, O] {
    ...
    def |>[O2](p: Process[O, O2]): Process[I, O2] = p match {
      ...
      case Await(f) => this match {
        ...
        case Await(g) => Await((i: Option[I]) => g(i) |> p)
      }
    }
     ...
     */
    def zipWithIndex: Process[I, (O, Int)] = for {
      value <- this
      i <- count
    }
      yield (value, i - 1)
  ...
  object Process {
    ...
    implicit def toMonadic[I, O](a: Process[I, O]): Monadic[I, I] = monad[I].toMonadic(a)
    ...
    def mean: Process[Double, Double] = {
      ...
      def go(acc: Double, n: Int): Process[Double, Double] =
        await(i => emit((acc + i) / n, go(acc + i, n + 1)))
    ...
    def sum2: Process[Double, Double] = loop(0.0)((i, acc) => (i + acc, i + acc))

    def count3[I]: Process[I, Int] = loop(1)((_, acc) => (acc, acc + 1))
  ...
  trait Process[F[_], O] {
    ...
    def tee[O2, O3](p2: Process[F, O2])(t: Tee[O, O2, O3]): Process[F, O3] = {
      t match {
        ...
        case Emit(h, tail) => Emit(h, (this tee p2)(tail))
